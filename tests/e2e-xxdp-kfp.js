...
async function run() {
  ...
  await xxdp.launchDiagnostic({
    mach, ev,
    command: "R KFPAD0",
    // Same reasoning as EKBBF0: the budgets are sized from the measured
    // timings (self-paced diagnostic, banner in tens of seconds at most), not
    // from a guess about how slow a runner might be.
